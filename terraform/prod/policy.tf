...
resource "aws_iam_policy" "pdex_rds_proxy_secrets_policy" {
  name = "pdex-cluster-pdex-secrets-Prod"
    ...
    Statement = [
      {
        ...
        Resource = [
          ...
          "arn:aws:secretsmanager:ca-central-1:868987904026:secret:secrets.env-YejdSZ*"
        ]
      }
    ]
  })
...
resource "aws_iam_role" "pdex_rds_proxy_secrets_role" {
  name = "${aws_eks_cluster.pdex-cluster.name}-rds-proxy-secrets-Prod"
...
resource "aws_iam_role" "pdex_pods_secrets_role" {
  name = "${aws_eks_cluster.pdex-cluster.name}-pdex-secrets-Prod"
